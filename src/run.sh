...
case "${sol#*.}" in
	cpp)
		...
		g++ solution.cpp -c -O3 -Wall -Wunreachable-code
...
(
	...
	time -o score -f "wall=%e sys=%S usr=%U cpu=%P mmax=%M rssavg=%t mavg=%t pvt=%D ss=%p ts=%X maj=%F min=%R swp=%W iow=%w in=%I out=%O" timeout "$wall_secs" $run <"/data$test_path" >stdout #2>/dev/null
	echo $?
)
