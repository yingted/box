...
do
	...
	{ cat rootfs/tmp/score 2>/dev/null && touch rootfs/tmp/stdout && rootfs/build/score "$data_dir$(sed 's/\(.*\)\binput\b/\1output/' <<< "$test_path")" rootfs/tmp/stdout; } > "$file/score.part" && mv "$file/score.part" "$file/score" || { >> "$file/score"; rm -f "$file/score.part"; }
